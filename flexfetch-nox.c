//usr/bin/gcc flexfetch-nox.c `pkg-config gtk+-3.0 --cflags --libs` -lfontconfig -lm -g -o flexfetch-nox; exec ./flexfetch-nox
...
char* fetch_gtk_theme() {
	//return "Dracula (TODO)";
	gchar* gtk_theme;
	g_object_get(gtk_settings_get_default(), "gtk-theme-name", &gtk_theme, NULL);
	return gtk_theme;
}
